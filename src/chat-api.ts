...
export const createChatApi = (store: ChatStore) => async (request: Request): Promise<Response> => {
    ...
    try {
        ...
        if (request.method === "POST") {
            ...
            if (voiceMatch) {
                if (!voiceMatch[2]) return Response.json(await store.voice.start(voiceMatch[1]!, body.id, body.sdp, body.voice, request.signal, body.listenOnly), { headers: { "Cache-Control": "no-store" } })
                if (voiceMatch[3] === "respond") return Response.json(store.voice.respond(voiceMatch[1]!, voiceMatch[2]), { headers: { "Cache-Control": "no-store" } })
