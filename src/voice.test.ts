...
test("listen-only buffers fragments, blocks unwanted speech and tasks, then releases once", async () => {
    const f = fixture()
    const result = await f.store.start("chat", crypto.randomUUID(), sdp, "cove", new AbortController().signal, true)
    expect(JSON.parse(String(f.requests[0]!.init.body)).session.instructions).toContain("LISTEN ONLY")
    const fragment = { type: "input_transcript.added", item: { id: "fragment-1", text: "I want to think about this. " } }
    f.socket().event(fragment); f.socket().event(fragment)
    f.socket().event({ type: "input_transcript.added", item: { id: "fragment-2", text: "Here is the rest of my monologue." } })
    f.socket().event(delegation("suppressed", "execute this immediately"))
    f.socket().event({ type: "turn.done", turn: { id: "unwanted", role: "assistant", transcript: "Unwanted reply" } })
    expect(f.consulted).toHaveLength(0)
    expect(f.transcripts).toHaveLength(0)
    expect(f.socket().sent).toHaveLength(0)
    expect(f.store.get("chat", result.id)).toMatchObject({ listenOnly: true, partialTranscript: "I want to think about this. Here is the rest of my monologue." })
    expect(() => f.store.respond("other", result.id)).toThrow("not found")
    f.store.respond("chat", result.id); f.store.respond("chat", result.id)
    expect(f.socket().sent).toHaveLength(1)
    expect(f.socket().sent[0]).toMatchObject({ type: "session.context.append", channel: "speakable" })
    expect(f.store.get("chat", result.id).listenOnly).toBe(false)
    f.socket().event(delegation("suppressed", "execute this immediately"))
    expect(f.consulted).toHaveLength(0)
    f.socket().event(delegation("fresh", "Respond to my thoughts"))
    await Bun.sleep(5)
    expect(f.consulted).toHaveLength(1)
    expect(f.consulted[0]).toContain("Here is the rest of my monologue.")
    const turn = { type: "turn.done", turn: { id: "user-turn", role: "user", transcript: "My complete monologue" } }
    f.socket().event(turn); f.socket().event(turn)
    expect(f.transcripts).toEqual([["chat", "user", "My complete monologue"]])
    expect(f.store.get("chat", result.id).partialTranscript).toBe("")
    f.store.stop("chat", result.id)
    expect(() => f.store.respond("chat", result.id)).toThrow("not connected")
})

test("ending listen-only saves unfinalized words without launching a task", async () => {
    const f = fixture()
    const result = await f.store.start("chat", crypto.randomUUID(), sdp, "cove", new AbortController().signal, true)
    f.socket().event({ type: "input_transcript.added", item: { id: "f", text: "Unfinished thoughts" } })
    f.store.stop("chat", result.id); f.store.stop("chat", result.id)
    expect(f.transcripts).toEqual([["chat", "user", "Unfinished thoughts"]])
    expect(f.consulted).toHaveLength(0)
})
