...
test("chats can be created with a custom title", async () => {
    const { cwd } = await fixture()
    const store = new ChatStore(new Database(":memory:"), cwd)
    stores.push(store)
    const api = createChatApi(store)
    const created = await store.create(undefined, undefined, undefined, undefined, undefined, "Discord inbox")
    expect(created.title).toBe("Discord inbox")
    expect(store.get(created.id)?.title).toBe("Discord inbox")
    await expect(store.create(undefined, undefined, undefined, undefined, undefined, "  ")).rejects.toThrow("chat name")
    const response = await api(new Request("http://localhost/api/chats", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Bot alerts" }) }))
    expect(response.status).toBe(201)
    expect((await response.json() as { title: string }).title).toBe("Bot alerts")
    const bad = await api(new Request("http://localhost/api/chats", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "" }) }))
    expect(bad.status).toBe(400)
})

test("chat titles can be changed and persist", async () => {
    const { cwd } = await fixture()
    const database = join(cwd, "chats.sqlite")
    const store = new ChatStore(new Database(database), cwd)
    stores.push(store)
    const chat = await store.create()
    const api = createChatApi(store)
    const rename = (title: unknown) => api(new Request(`http://localhost/api/chats/${chat.id}/title`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title }),
    }))

    const response = await rename("  Project puppy  ")
    expect(response.status).toBe(200)
    expect((await response.json() as { title: string }).title).toBe("Project puppy")
    ...
    stores.splice(stores.indexOf(store), 1)
    await store.close()
    const reopened = new ChatStore(new Database(database), cwd)
    stores.push(reopened)
    expect(reopened.get(chat.id)?.title).toBe("Project puppy")
})
...
test("chat GPT settings preserve history and context, persist, clear, and reject active turns", async () => {
    const { cwd, authFile } = await fixture()
    const path = join(cwd, "gpt-settings.sqlite")
    const store = new ChatStore(new Database(path), cwd, { authFile, fetchImpl: async () => sse([answer("Original reply")]) })
    const chat = await store.create(cwd)
    store.send(chat.id, "Remember this conversation")
    await store.settled()
    const before = await store.context(chat.id)
    const gpt = store.gpts.save({ name: "Support", description: "Support agent", instructions: "Use concise support replies.", model: "gpt-5.6-sol", reasoningEffort: "high" })
    const api = createChatApi(store)
    const change = (body: unknown) => api(new Request(`http://localhost/api/chats/${chat.id}/gpt`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }))
    expect((await change({ gptId: gpt.id })).status).toBe(200)
    expect(store.get(chat.id)?.gpt?.name).toBe("Support")
    expect(store.list().find(item => item.id === chat.id)?.gptName).toBe("Support")
    const after = await store.context(chat.id)
    expect(after.snapshot?.items).toEqual(before.snapshot?.items)
    expect(after.instructions).toContain(gpt.instructions)
    expect(after.instructions).not.toContain("Your name is PuppyGPT")
    expect(after.instructions).not.toContain("You are an assistant")
    expect(after.reasoningEffort).toBe("high")
    expect(store.get(chat.id)?.messages.map(m => m.text)).toEqual(["Remember this conversation", "Original reply"])
    expect((await change({ gptId: "missing" })).status).toBe(400)
    expect((await change({})).status).toBe(400)
    store.get(chat.id)!.status = "running"
    expect(() => store.setGpt(chat.id, null)).toThrow("Wait for")
    store.get(chat.id)!.status = "idle"
    await store.close()
    const reopened = new ChatStore(new Database(path), cwd, { authFile })
    stores.push(reopened)
    expect(reopened.get(chat.id)?.gpt?.id).toBe(gpt.id)
    expect((await reopened.context(chat.id)).snapshot?.items).toEqual(before.snapshot?.items)
    reopened.setGpt(chat.id, null)
    expect(reopened.get(chat.id)?.gpt).toBeUndefined()
    expect(reopened.get(chat.id)?.model).toBe(gpt.model)
    expect((await reopened.context(chat.id)).instructions).not.toContain(gpt.instructions)
    expect((await reopened.context(chat.id)).instructions).toContain("Your name is PuppyGPT")
})
