...
export function App() {
    ...
    useEffect(() => {
        const sync = () => {
            ...
            if (next.settings || next.environments || next.chatId) history.replaceState({ chatId: next.chatId }, "", next.environments ? `/environments${next.environmentId ? `/${next.environmentId}` : ""}` : next.settings ? `/settings/${next.section}` : chatPath(next.chatId))
    ...
    useEffect(() => {
        api<AppConfig>("/api/config").then(value => { setConfig(value); setCwd(value.cwd); setModel(value.settings.model); setExecutionTarget(value.execution.defaultEnvironmentId) }).catch(error => setError(error.message))
    }, [])
    ...
    const send = async (event: FormEvent) => {
        ...
        setError("")
        try {
            const current = chat ?? await api<Chat>("/api/chats", { cwd, model, environmentId: executionTarget || undefined })
    ...
    return <div className={`app ${sidebarOpen ? "sidebar-open" : "sidebar-closed"}`}>
        ...
        <aside className="sidebar" inert={!sidebarOpen} aria-hidden={!sidebarOpen}>
            ...
            <button disabled={sending || changingTarget} className={`new-chat ${!selectedId && !settingsOpen && !environmentsOpen ? "selected" : ""}`} onClick={() => select(null)}><Icon name="plus" size={18} /><span>New chat</span><kbd>Ctrl K</kbd></button>
            ...
            <div className={`composer-region ${!selectedId ? "welcome-composer" : ""}`}>
                ...
                <form className="composer" onSubmit={send}>
                    ...
                    <div className="composer-toolbar"><div className="composer-options">
                        ...
                        <span className="toolbar-divider" />
                        <label className="model-picker execution-picker"><span className="execution-label">Environment</span><select aria-label="Environment" value={chat ? chat.environmentId ?? `default-${chat.executionTarget ?? config?.execution.defaultTarget}` : executionTarget} disabled={!config || loading || sending || running || changingTarget} onChange={event => void changeExecutionTarget(event.target.value)}>
                            {chat?.environmentId && !config?.execution.environments.some(e => e.id === chat.environmentId) && <option value={chat.environmentId}>Unavailable environment</option>}
                            {config?.execution.environments.map(e => <option key={e.id} value={e.id}>{e.name}{e.status !== "ready" ? ` · ${e.status}` : ""}</option>)}
                        </select></label><button type="button" className="workspace-button" onClick={openEnvironments}>Manage</button>
